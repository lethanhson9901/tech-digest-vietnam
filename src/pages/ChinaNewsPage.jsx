...
import ChinaNewsSection from '../components/ChinaNewsSection';
...
const ChinaNewsPage = () => {
  ...
  useEffect(() => {
    const loadChinaNews = async () => {
      try {
        ...
        // Parse the sources from the response content
        if (data && data.content) {
          // Extract all sources from the content
          const sourceMatches = data.content.match(/\[SOURCE\] ([^\|]+?)\s*\|\s*([^\n]+)/g);

          if (sourceMatches) {
            const sources = sourceMatches.map(match => {
              const [, name, title] = match.match(/\[SOURCE\] ([^\|]+?)\s*\|\s*([^\n]+)/);
              return {
                name: name.trim(),
                title: title.trim(),
                languageName: name.trim() // In a real scenario, you might map this to Vietnamese names
              };
            });

            setNewsData({ sources, rawContent: data.content });
          } else {
            // Fallback to known sources if parsing fails
            setNewsData({
              sources: [
                { name: 'toutiao', title: '今日头条', languageName: 'TouTiao' },
                { name: 'baidu', title: '百度热搜', languageName: 'Baidu' },
                { name: 'wallstreetcn-hot', title: '华尔街见闻', languageName: 'Wall Street CN Hot' },
                { name: 'thepaper', title: '澎湃新闻', languageName: 'The Paper' },
                { name: 'bilibili-hot-search', title: 'bilibili 热搜', languageName: 'Bilibili Hot Search' },
                { name: 'cls-hot', title: '财联社热门', languageName: 'CLS Hot' },
                { name: 'zhihu-daily', title: '知乎日报', languageName: 'Zhihu Daily' },
                { name: 'weibo-hot', title: '微博热搜', languageName: 'Weibo Hot' },
                { name: 'tieba-hot', title: '贴吧热门', languageName: 'Tieba Hot' },
                { name: 'toutiao-scitech', title: '今日头条科技', languageName: 'TouTiao SciTech' },
                { name: 'netease-news', title: '网易新闻', languageName: 'Netease News' }
              ],
        ...
        setError(err.message || 'Failed to load China news');
      } finally {
        setIsLoading(false);
      }
    };
  ...
  if (!newsData) {
    return <ErrorMessage message="China news data not found" />;
  }
  ...
  return (
    <div className={`max-w-6xl mx-auto px-4 ${prefersReducedMotion ? '' : 'transition-all duration-700 transform'} ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
      ...
      <div className="mb-10 text-center relative">
        ...
        <div className="relative">
          ...
          <p className="text-xl md:text-2xl max-w-3xl mx-auto leading-relaxed mb-8 text-secondary">
            🇨🇳 Cập nhật <span className="font-semibold text-primary-600 dark:text-dark-accent-primary-bg">tin tức Trung Quốc</span> mới nhất từ các nguồn uy tín
          </p>
      ...
      <div className="space-y-8 mb-14">
        {newsData.sources.map((source, index) => (
          <ChinaNewsSection
            ...
            source={source}
            index={index}
            rawContent={newsData.rawContent}
          />
        ))}
      </div>
      ...
      <div className="bg-neutral-50 dark:bg-[#020817] rounded-3xl p-8 md:p-12 mb-16">
        <h2 className="text-3xl font-bold text-center mb-12 text-primary">Tại sao chọn Tin Trung Quốc?</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          <div className="text-center">
            ...
            <p className="text-secondary text-sm">Thông tin được tổng hợp từ các nguồn uy tín Trung Quốc</p>
          </div>
          <div className="text-center">
            ...
            <p className="text-secondary text-sm">Tin tức mới được cập nhật hàng ngày</p>
          </div>
          <div className="text-center">
          ...
          <div className="text-center">
            ...
            <p className="text-secondary text-sm">Phân tích chi tiết từ nhiều nguồn khác nhau</p>
          </div>
        </div>
      </div>
    </div>
